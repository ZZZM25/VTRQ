    ...
    if node.is_leaf():
        ...
        for e in node.adjacent_list:
            ...
            for i in range(len(traj[1]) - 1):
                e1_start = traj[1][i][0]
                e1_end = traj[1][i + 1][0]
                e_start_time = traj[1][i][-1]
                e_end_time = traj[1][i + 1][-1]
                # 如果从轨迹中拿出来的一条边和列表中的这条边有重合证明轨迹经过这条边
                ...
                if (e.start.id == e1_start and e.end.id == e1_end) or (e.start.id == e1_end and e.end.id == e1_start):
                    ...
                    hash_object = hashlib.sha256(encoded_data)
                    hash_hex = hash_object.hexdigest()
                    trajectory = Traj(e_start_time, e_end_time, hash_hex)
                    start_time_str = str(e_start_time)
                    end_time_str = str(e_end_time)
                    str_all = start_time_str + end_time_str + trajectory.traj_hash
                    encoded_data = str_all.encode('utf-8')
                    hash_object = hashlib.sha256(encoded_data)
                    hash_hex = hash_object.hexdigest()
    ...
    if not node.is_leaf():
        ...
        for e in node.linking:
            ...
            for i in range(len(traj[1]) - 1):
                ...
                if (e.start.id == e1_start and e.end.id == e1_end) or (
                        e.start.id == e1_end and e.end.id == e1_start):
                    traj_start_time = traj[1][0][-1]
                    traj_end_time = traj[1][-1][-1]
                    traj_start_time_str = str(traj_start_time)
                    traj_end_time_str = str(traj_end_time)
                    ...
                    trajectory.merge_hash = hash_hex
                    index = 0
                    while index < len(e.traj_hashList) and e.traj_hashList[
                        index].start_time <= trajectory.start_time:
                        index = index + 1
                    e.traj_hashList.insert(index, trajectory)
                    # print("找到", e.start.id, e.end.id)
