...
class Vertex:
    def __init__(self, id, lat, lng):
        ...
        self.lat = lat
        self.lng = lng
        self.min_timestamp = 10000000000000
...
network_id = set()
def traverse_all_edges(node):
    if node is None:
        return

    if not node.is_leaf():
        for e in node.linking:
            network_id.add(e.start)
            network_id.add(e.end)
    else:
        for e in node.adjacent_list:
            network_id.add(e.start)
            network_id.add(e.end)
    ...
    # 递归遍历左子树
    traverse_all_edges(node.left)

    # 递归遍历右子树
    traverse_all_edges(node.right)


def collect_points(csv_file, traj_id_set):
    """
    从CSV文件中收集指定轨迹ID对应的所有点

    参数:
        csv_file: CSV文件路径
        traj_id_set: 包含需要查询的traj_id的集合
    ...
    with open(csv_file, mode='r', encoding='utf-8') as file:
        ...
        for row in csv_reader:
            traj_id = row['traj_id']
            # 检查当前行的traj_id是否在目标集合中
            if traj_id in traj_id_set:
                # 获取first_elements并按|分割
                elements = row['first_elements']
                points = elements.split('|')
                # 将分割后的点添加到集合中
                points_set.update(points)
...
if __name__ == "__main__":
    ...
    # print(len(interval_set))
    # # CSV文件路径
    # csv_file_path = 'trajectories_chengdu.csv_process.csv'  # 替换为你的CSV文件路径
    # # 收集所有点
    # result_points = collect_points(csv_file_path, interval_set)
    #
    # # 输出结果
    # print(f"所有轨迹经过的点共有 {len(result_points)} 个（已去重）：")
    # print(result_points)
    # end_time = time.time()
